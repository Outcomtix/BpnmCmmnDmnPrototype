...
window.addEventListener("DOMContentLoaded", () => {
  (async function () {
    ...
    try {
      ...
      if (!DmnModelerCtor) {
        console.error(
          "DMN modeler constructor not found. Available DMN globals:",
          {
        ...
        console.warn(
          "DMN modeler library not loaded (or unexpected global). DMN functionality will be disabled."
        );
      } else {
    ...
    function renderProps(root, entries) {
      ...
      (entries || []).forEach(([k, v]) => {
        ...
        if (
          typeof v === "string" &&
          (v.startsWith("bpmn:") || v.startsWith("dmn:"))
        ) {
    ...
    function wirePropsCMMN() {
      ...
      bus.on("element.click", (e) => {
        const def =
          e.element?.businessObject?.definitionRef || e.element?.businessObject;
    ...
    function wirePropsBPMN() {
      ...
      bus.on("element.click", (e) => {
        ...
        const jump = bo?.extensionElements?.values?.find((v) =>
          v.$type?.includes("jump")
        );
    ...
    async function ensureDmnOpenView() {
      ...
      const chosen =
        preferredOrder.map((t) => byType[t]).find(Boolean) || views[0];
    ...
    $("#btn-token").addEventListener("click", async () => {
      ...
      for (const el of path) {
        const gfx = bpmnModeler
          .get("graphicsFactory")
          ._elementRegistry.getGraphics(el);
